...
reg_A1 <- c(max(reg_1$count),mean(reg_1$count))
reg_A2 <- c(max(reg_2$count),mean(reg_2$count))
reg_A3 <- c(max(reg_3$count),mean(reg_3$count))
reg_A4 <- c(max(reg_4$count),mean(reg_4$count))
reg_A5 <- c(max(reg_5$count),mean(reg_5$count))
regA<- cbind(reg_A1, reg_A2, reg_A3, reg_A4, reg_A5)
regA<- t(regA)
colnames(regA) <- c("max","mean")
regA%>%
  kable()
...
reg_1 <- cbind(reg_1$count, reg_11)
reg_2 <- cbind(reg_2$count, reg_21)
reg_3 <- cbind(reg_3$count, reg_31)
reg_4 <- cbind(reg_4$count, reg_41)
reg_5 <- cbind(reg_5$count, reg_51)

regB <- rbind(reg_1, reg_2, reg_3, reg_4, reg_5)
colnames(regB) <- c("count", "station")
