...
data1.pcaS <- data.frame(unclass(summary(data.pca)$importance))
data1.pcaS %>% 
  kable()
```
...
data_MM.pca <- prcomp(data_MM[,-1], scale=T)

data_MU <- data_MU[,c(-20:-22)]
data_MU.pca <- prcomp(data_MU[,-1], scale=T)

data_FM <- data_FM[,c(-20:-22)]
data_FM.pca <- prcomp(data_FM[,-1], scale=T)

data_FU <- data_FU[,c(-20:-22)]
data_FU.pca <- prcomp(data_FU[,-1], scale=T)
...
```{r 情報1}
data31 <- data1.frame(house_num=data$house_num, Q12=data1$`12`, Q18=data1$`18`, Q8=data1$`8`, Q9=data1$`9`,Q4=data1$`4`, Q7=data1$`7`, Q14=data1$`14`, Q15=data1$`15`, Q16=data1$`16`, sex = data1$sex, marriage =data$marriage, age = data$age )
