...
library(proxy)
library(MASS)
source("/home/miidri/re1/psql.R")
source("/home/miidri/re1/name_data.R")
```
...
scoresIS_new <- cbind(scoresIS_new, index, genreIS, age_index)

```

## 男女・未婚既婚別
```{r}
S12 <- ggplot(scoresIS_new, aes(x=ML1, y=ML2, col=index)) +
  geom_point(size = 0.5)+
...
scoresIS <- cbind(scoresIS, index, age_index)
